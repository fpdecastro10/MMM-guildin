...
def main():
    ...
    with st.sidebar:
        ...
        df_stores = pd.DataFrame(dataframe_coefficients).sort_values("Tendencia de venta", ascending=False).head(10)

        stores_filter = df_stores["Store id"]    
        relations_storesDB_storeId = df_stores[
            ["stores", "Store id"]
        ].drop_duplicates(["stores", "Store id"])
        store_selected = st.selectbox(
            "Stores con tendencia positiva:", stores_filter
        )
    ...
    df_stores_formated = df_stores.to_html(
        index=False
    ).replace(
        "<td>",
        ...
        "Tendencia de venta",
        "<span class='enlace'>Tendencia de ventas &#9432</span>"
    )
