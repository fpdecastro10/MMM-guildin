...
data_sw = pd.read_csv("datasets/datasetCampignSales.csv")
data_sw["concat_store_group_name"] = data_sw["store_group_id"].astype(str) + " - " + data_sw["name_storeGroup"]

data_sw_1 = pd.read_csv("datasets/datasetCampignSalesNew.csv")
data_sw_1["concat_store_group_name"] = data_sw_1["store_group_id"].astype(str) + " - " + data_sw_1["name"]

st.session_state.simulation_run = False

data_sales_stores = pd.read_csv(
    "datasets/dataset_2_Week_later_salesmorethan0.csv"
)
if 'df_sales_storeGroup' in st.session_state:
    df_sales_storeGroup = st.session_state.df_sales_storeGroup
else:
    st.session_state.df_sales_storeGroup = pd.read_csv(
        "datasets/dataset_to_detect_performance_of_stores.csv"
    )
    df_sales_storeGroup = st.session_state.df_sales_storeGroup

list_campaing_store_group = data_sw["campaign"].unique().tolist()
list_campaing_store_group_new_client = df_sales_storeGroup.query(
    "campaign_storeGroup not in @list_campaing_store_group"
)["campaign_storeGroup"].unique().tolist()

unique_combinations = data_sw[[
    "store_group_id",
    "name_storeGroup",
    "campaign"
]].drop_duplicates(["store_group_id", "name_storeGroup", "campaign"])

unique_combinations_new_client = df_sales_storeGroup[[
                                    "id_storeGroup",
                                    "name_storeGroup",
                                    "campaign_storeGroup"]].drop_duplicates(
                                        [
                                            "id_storeGroup",
                                            "name_storeGroup",
                                            "campaign_storeGroup"
...
def second_approach(input_number,list_seleceted_campaign,bool_execute):
    if bool_execute:
        ...
        for sg_list in dict_shap.keys():
            sales_with_investment_sg[sg_list] = sale_simulation_sg(
                sg_list,
                dict_investment_media,
                datetime.today()
            )
...
def third_percent(selectedGroup):
    ...
    for store in list_storeGroups_id:
        df_storeGroup_id = data_sw[
            data_sw["concat_store_group_name"] == store
        ].query("sales > 0")
...
def new_client(camaping_new_client, investment, selectedStoreGroups, dict_shap):
    ...
    pie_graph(
        pd.DataFrame(set_coeffs.items(), columns=['canal', 'Inversión ($)']),
        "canal",
        "Inversión ($)",
        "Distribución del budget total por canal"
    )
    pie_graph(
        df_filter_by_camp_time, "Store Group",
        "Inversión ($)",
        "Distribución del budget total por Store Group"
    )
...
def main():
    ...
    with st.sidebar:
        ...
        type_of_client = st.selectbox(
            "Seleccione el tipo de campaña:",
            ["Campaña Nueva", "Campaña con Historial"]
        )

        if type_of_client == "Campaña Nueva":
            camaping_new_client = st.selectbox(
                "Seleccione el nombre de la campaña: ",
                list_campaing_store_group_new_client
            )
            list_store_group_coeff = ["Seleccionar todos"]
            list_store_group_coeff += data_sw[
                'concat_store_group_name'
            ].unique().tolist()

        else:
            selected_option_campaign = st.selectbox(
                "Seleccione el nombre de la campaña: ",
                list_campaing_store_group
            )
            investment_in_media = st.number_input(
                "Inversión en medios:",
                min_value=0
            )
    ...
    if type_of_client == "Campaña Nueva":
        ...
        number_input_increases = st.number_input(
            "Ingrese el crecimiento porcentual deseado por semana (%)",
            min_value=0,
            value=0
        )
