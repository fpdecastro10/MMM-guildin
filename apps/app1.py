...
def def_delay_dataset(delay):
    if delay == '1 semana':
        data_sw = pd.read_csv('datasets/dataset_1_Week_later_salesmorethan0.csv')
    else:
        data_sw = pd.read_csv('datasets/dataset_2_Week_later_salesmorethan0.csv')
...
def main():
    with st.sidebar:
        ...
        optionOfNameCampaign = unique_combinations[
            "campaign_storeGroup"
        ].drop_duplicates().tolist()

        opciones_seleccionadas = st.selectbox(
            "Seleccione el nombre de la campaña",
            optionOfNameCampaign
        )

        unique_combinationsStore = unique_combinations.query(
            "campaign_storeGroup in @opciones_seleccionadas"
        )
        index_storeGroup = {}
        ...
        for key, value in temp_index_storeGroup.items():
            if data_sw.query(
                f"id_storeGroup == {value}"
            ).groupby("yearweek_campaign").sum().reset_index().shape[0] <= 0:
        ...
        if index_storeGroup == {}:
            ...
            filtros_seleccionados = st.radio(
                'Seleccione la granularidad de tiempo:',
                opciones
            )
            numero_ingresado = st.number_input(
                "Ingrese el monto de campaña a invertir",
                value=0.0,
                step=0.1
            )
        ...
        if index_storeGroup != {}:
            ...
            min_value_calculated = min(
                filter_data_storeGroup['yearweek_campaign']
            )
            max_value_calculated = max(
                filter_data_storeGroup['yearweek_campaign']
            )
        ...
        free_grades = st.number_input(
            "Ingrese el número de grados de la regresión",
            value=2
        )
    ...
    if index_storeGroup != {}:
        ...
        filter_data_storeGroup = data_sw.query(
            f"id_storeGroup == {index_storeGroup[selected_filter]}"
        )

        filtered_data = filter_data_storeGroup.groupby(
            "yearweek_campaign"
        ).agg({'sales': 'sum', 'cost_campaign': 'mean'}).reset_index()
        promedio_ventas = np.mean(
            filtered_data.query(
                f"{start_date} < yearweek_campaign and yearweek_campaign < {end_date}"
            )['sales']
        )
        ...
        filtered_data_product = filter_data_storeGroup.groupby(
            ...
            ['id_sku', 'name_product', 'sales']
        ]
        total_sales = filtered_data_product['sales'].sum()
        filtered_data_product['share'] = filtered_data_product['sales']/total_sales

        # 81
        filtered_data_product_store = filter_data_storeGroup.groupby(
            ['id_sku', 'id_store_retailer', 'name_retailer']
        ).sum().reset_index()[
            ['id_sku', 'id_store_retailer', 'name_retailer', 'sales']
        ]
        filtered_data_product_store['share'] = filtered_data_product_store['sales']/total_sales

        d = np.polyfit(
            filtered_data['cost_campaign'],
            filtered_data['sales'],
            free_grades
        )
        f = np.poly1d(d)
        ...
        function_valuated = pd.DataFrame(
            {'cost_campaign': interval_values, 'Treg': result_values}
        )

        function_valuated.plot(
            x='cost_campaign',
            y='Treg',
            color='red',
            legend=False,
        ...
        if numero_ingresado == 0:
            st.markdown("#### Coloque un monto a calcular")
        else:
            ...
            filtered_data_product.rename(
                columns={'share_sales': 'Qty sales (un)'},
                inplace=True
            )
            filtered_data_product.rename(
                columns={'id_sku': 'Sku id'},
                inplace=True
            )
            filtered_data_product.rename(
                columns={'name_product': 'Producto'},
                inplace=True
            )
            st.markdown(
                "<h2 style=color:#f7dc00> Detalle por producto: </h2>",
                unsafe_allow_html=True
                )

            st.markdown(dataframe_to_markdown_str(
                filtered_data_product[
                    ['Sku id', 'Producto', 'Qty sales (un)']
                ])
            )
            filtered_data_product_store.rename(
                columns={'id_sku': 'Sku id'},
                inplace=True
            )
            filtered_data_product_store.rename(
                columns={'id_store_retailer': 'Store id'},
                inplace=True
            )
            filtered_data_product_store.rename(
                columns={'name_retailer': 'Retailer'},
                inplace=True
            )
            filtered_data_product_store.rename(
                columns={'share_sales': 'Qty sales (un)'},
            ...
            st.markdown(
                "<h2 style=color:#f7dc00> Detalle por producto y store: </h2>",
                unsafe_allow_html=True
            )
            st.markdown(dataframe_to_markdown(filtered_data_product_store[
                ['Sku id', 'Store id', 'Retailer', 'Qty sales (un)']
            ].sort_values(by='Qty sales (un)', ascending=False)))
