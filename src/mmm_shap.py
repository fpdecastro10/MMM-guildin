...
def df_builder_tablaMedio(df, list_group, dict_group):
    ...
    pivot_table_tablaMdio_cost = pd.pivot_table(
        ...
        index=['ISOweek', 'concat_store_group_name'],
        columns="tabla_medio"
    )
    isoWeek_sales_origin = df_tablaMedio_ISOweek[
        ['ISOweek', 'concat_store_group_name', 'sales']
    ].groupby(
        ['ISOweek', 'concat_store_group_name']
    ).mean().reset_index()
    union_sales_tablaMedio_cost = pd.merge(
        pivot_table_tablaMdio_cost,
        isoWeek_sales_origin,
        on=['ISOweek', 'concat_store_group_name'],
        how='left'
    )
...
def calculated_shape_values(list_sg=list_store_group):
    ...
    for store_group_index in range(len(list_sg)):
        ...
        data_sg = data_whole_sg_wp[
            data_whole_sg_wp['concat_store_group_name'] == store_group
        ]
        ...
        if os.path.exists(f"models/trained_models/{store_group}.pkl"):
...
def calculated_increment_sales(
        model,
        growing,
        shap_values,
...
def sale_simulation_sg(sg, dict_investment_media, date_to_estimate):
    ...
    with open(f"models/trained_models/{sg}.pkl", 'rb') as f:
        prophet = pickle.load(f)
