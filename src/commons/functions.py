...
def rssd(effect_share, spend_share):
    ...
    Decomposition distance (root-sum-square
    distance, a major innovation of Robyn)
    eliminates the majority of "bad models"
    (larger prediction error and/or unrealistic
    media effect like the smallest channel
    getting the most effect)
...
def calculate_spend_effect_share(df_shap_values: pd.DataFrame, media_channels, df_original: pd.DataFrame):
    ...
    spends_percentages = pd.DataFrame(
        df_original[media_channels].sum(axis=0) / df_original[media_channels].sum(axis=0).sum(), columns=["spend_share"]
    )
    spends_percentages
...
def model_refit(data, target, features, media_channels, model_params, adstock_params, start_index, end_index):
...
def plot_shap_vs_spend(
    df_shap_values, x_input_interval_nontransformed, x_input_interval_transformed, features, media_channels, figsize=(25, 10)
):
    for channel in media_channels:
        ...
        sns.regplot(
            x=x_input_interval_transformed[channel],
            y=df_shap_values[channel],
            label=channel,
            scatter_kws={"alpha": 0.65},
            line_kws={"color": "C2", "linewidth": 6},
            lowess=True,
            ax=ax,
    ...
    for train_index, test_index in tscv.split(data_temp):
        ...
        if is_multiobjective:
            ...
            spend_effect_share = calculate_spend_effect_share(
                df_shap_values=df_shap_values, media_channels=media_features, df_original=data.iloc[train_index]
            )

            decomp_rssd = rssd(
                effect_share=spend_effect_share.effect_share.values, spend_share=spend_effect_share.spend_share.values
            )
...
def df_builder_tablaMedio(df, list_group, dict_group):
    df_tablaMedio_ISOweek = df.groupby(list_group).agg(dict_group).reset_index()

    pivot_table_tablaMdio_cost = pd.pivot_table(
        df_tablaMedio_ISOweek,
        index=["ISOweek", "ISOweek_wf", "concat_store_group_name", "campaign"],
        values="cost_campaign",
        columns="tabla_medio",
    )
    isoWeek_sales_origin = (
        df_tablaMedio_ISOweek[["ISOweek", "ISOweek_wf", "concat_store_group_name", "campaign", "sales"]]
        .groupby(["ISOweek", "ISOweek_wf", "concat_store_group_name", "campaign"])
        .mean()
        .reset_index()
    )
    union_sales_tablaMedio_cost = pd.merge(
        pivot_table_tablaMdio_cost,
        isoWeek_sales_origin,
        on=["ISOweek", "ISOweek_wf", "concat_store_group_name", "campaign"],
        how="left",
    )
...
def calculated_incerement_sales(model, growing, shap_values, data_input_nontransformed, data_store_group_wi, features):
    ...
    if not isinstance(shap_values_scope, pd.DataFrame):
    ...
    dataframe = pd.DataFrame({"ds": [date_to_estimate]})
